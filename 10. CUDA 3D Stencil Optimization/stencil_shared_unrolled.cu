...
__global__ void opt_kernel(const double *in,
                           double *out,
                           uint64_t N)
{
...
int main()
{
  ...
  // --- (iii) Optimized Memory Kernel ---
  cout << "\n--- (iii) Optimized Kernel ---" << endl;
  ...
  cout << "Host to Device copy time (pageable):" << copy_host2Device << " ms\n";
  ...
  opt_kernel<<<gridDim, blockDim>>>(d_in, d_out, N);
  ...
  cout << "Device to Host copy time (pageable):" << copy_device2Host << " ms\n";
